<div id="sidebar" class="fl-left">
    <ul id="sidebar-menu">
        <li class="nav-item">
            <a href="" title="" class="nav-link nav-toggle">
                <i class="fa-solid fa-house"></i>
                <span class="title"> </i>Dashboards</span>
            </a>
            <ul class="sub-menu">
                <li class="nav-item">
                    <a href="?page=Dashboards" title="" class="nav-link">Thống kê</a>
                </li>

            </ul>
        </li>
        <li class="nav-item">
            <a href="" title="" class="nav-link nav-toggle">
                <span class="fa fa-map icon"></span>
                <span class="title">Trang</span>
            </a>
            <ul class="sub-menu">
                <li class="nav-item">
                    <a href="?page=add_page" title="" class="nav-link">Thêm mới</a>
                </li>
                <li class="nav-item">
                    <a href="?page=list_page" title="" class="nav-link">Danh sách các trang</a>
                </li>
            </ul>
        </li>
        <li class="nav-item">
            <a href="" title="" class="nav-link nav-toggle">
                <span class="fa fa-pencil-square-o icon"></span>
                <span class="title">Bài viết</span>
            </a>
            <ul class="sub-menu">
                <li class="nav-item">
                    <a href="?page=add_post" title="" class="nav-link">Thêm mới</a>
                </li>
                <li class="nav-item">
                    <a href="?page=list_post" title="" class="nav-link">Danh sách bài viết</a>
                </li>
                <li class="nav-item">
                    <a href="?page=list_cat" title="" class="nav-link">Danh mục bài viết</a>
                </li>
            </ul>
        </li>
        <li class="nav-item">
            <a href="" title="" class="nav-link nav-toggle">
                <span class="fa fa-product-hunt icon"></span>
                <span class="title">Sản phẩm</span>
            </a>
            <ul class="sub-menu">
                <li class="nav-item">
                    <a href="?page=add_product" title="" class="nav-link">Thêm mới</a>
                </li>
                <li class="nav-item">
                    <a href="?page=list_product" title="" class="nav-link">Danh sách sản phẩm</a>
                </li>
                <li class="nav-item">
                    <a href="?page=list_danhMucSanPham" title="" class="nav-link">Danh mục sản phẩm</a>
                </li>
            </ul>
        </li>
        <li class="nav-item">
            <a href="" title="" class="nav-link nav-toggle">
                <i class="fa-solid fa-cart-shopping"></i>
                <span class="title">Nhập hàng</span>
            </a>
            <ul class="sub-menu">
                <li class="nav-item">
                    <a href="?page=add_nhapHang" title="" class="nav-link">Nhập sản phẩm</a>
                </li>

                <li class="nav-item">
                    <a href="?page=nhapHang" title="" class="nav-link">Danh sách sản phẩm nhập</a>
                </li>
                <li class="nav-item">
                    <a href="?page=list_NCC" title="" class="nav-link">Danh sách nhà cung cấp</a>
                </li>

            </ul>
        </li>
        <li class="nav-item">
            <a href="" title="" class="nav-link nav-toggle">
                <span class="fa fa-database icon"></span>
                <span class="title">Bán hàng</span>
            </a>
            <ul class="sub-menu">
                <li class="nav-item">
                    <a href="?page=list_order" title="" class="nav-link">Danh sách đơn hàng</a>
                </li>

            </ul>
        </li>
        <li class="nav-item">
            <a href="#" title="" class="nav-link nav-toggle">
                <i class="fa-regular fa-user" aria-hidden="true"></i>
                <span class="title">Quản lý user</span>
            </a>
            <ul class="sub-menu">
                <li class="nav-item">
                    <a href="?page=list_salesStaff" title="" class="nav-link">Danh sách nhân viên</a>
                </li>
                <li class="nav-item">
                    <a href="?page=list_user" title="" class="nav-link">Danh sách người dùng</a>
                </li>
            </ul>
        </li>
    </ul>
</div>